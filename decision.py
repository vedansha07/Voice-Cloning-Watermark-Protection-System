def generate_verdict(deepfake_label: str, deepfake_score: float, watermark_confidence: float) -> dict:
    """
    Enhanced decision engine with security override rules.
    """
    ...
    # Rule 1: Strong AI Fake Override
    if normalized_label == "fake" and deepfake_score >= 0.9:
        return {
            "final_verdict": "Synthetic Audio",
            "risk_level": "High",
            "risk_score": round(ai_risk * 100, 2),
            "explanation": "AI model strongly indicates synthetic voice. Watermark presence cannot override high-confidence AI detection.",
            "ai_risk_component": round(ai_risk * 100, 2),
            "watermark_risk_component": round(watermark_risk * 100, 2),
            "conflict_type": "AI Override"
        }

    # Rule 2: Watermark Injection Suspicion
    if (
        normalized_label == "fake"
        and deepfake_score >= 0.8
        and watermark_confidence >= 0.8
    ):
        return {
            "final_verdict": "Watermark Injection Suspected",
            "risk_level": "High",
            "risk_score": round((ai_risk * 0.8 + watermark_risk * 0.2) * 100, 2),
            "explanation": "Synthetic voice detected with valid watermark. Possible post-generation watermark embedding attack.",
            "ai_risk_component": round(ai_risk * 100, 2),
            "watermark_risk_component": round(watermark_risk * 100, 2),
            "conflict_type": "Injection Suspected"
        }

    # Rule 3: Missing Watermark for Real Audio
    if (
        normalized_label == "real"
        and deepfake_score >= 0.8
        and watermark_confidence < 0.3
    ):
        return {
            "final_verdict": "Unauthenticated Audio",
            "risk_level": "Medium",
            "risk_score": round((ai_risk * 0.5 + watermark_risk * 0.5) * 100, 2),
            "explanation": "Audio appears human-generated but lacks expected watermark authentication.",
            "ai_risk_component": round(ai_risk * 100, 2),
            "watermark_risk_component": round(watermark_risk * 100, 2),
            "conflict_type": "Missing Watermark"
        }
